...
class BCNN(nn.Module):
    ...
    def __init__(self, in_channels=8, n_class=5, onnx_export=0):
        super().__init__()
    ...
    def forward(self, x):
        ...
        #_class_out = deconv0[:, 4:9, :, :]
        #pred_class = F.softmax(_class_out,dim=2)
        pred_class = F.softmax(deconv0[:, 4:9, :, :],dim=1)
        ...
        # print(height)
        if self.onnx_export:
            instance_pt = deconv0[:, 1:3, :, :]
            heading_pt = deconv0[:, 9:11, :, :]
            return category, instance_pt, confidence, pred_class, heading_pt, height

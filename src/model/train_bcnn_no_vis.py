...
class Trainer(object):
    ...
    def __init__(self, data_path, batch_size, max_epoch, pretrained_model,
        ...
        self.optimizer = optim.SGD(self.model.parameters(), lr=2e-6, momentum=0.5, weight_decay=1e-5)
        self.scheduler = optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lambda epo: 0.9 ** epo)
    ...
    def step(self, mode):
        ...
        for index, (in_feature, out_feature_gt) in tqdm.tqdm(
            ...
            if self.vis_on:
                ...
                #in_feature_img = get_category_or_confidence_image(in_feature[0,self.non_empty_channle:self.non_empty_channle+1,...],self.height, self.width, thresh=0.0)
                #in_feature_img = get_input_feature_image(in_feature[0,3:4,...],self.height, self.width, thresh=0.0)
        ...
        if np.mod(index, self.loss_print_interval) == 0:
            format_str = "total loss: {:3f}, category_loss: {:.3f}, confidence_loss: {:.3f}, class_loss:{:.3f}, \
            instace_x_loss:{:.3f}, instace_y_loss={:.3f}, heading_x_loss={:.3f}, heading_y_loss={:.3f}, height_loss={:.3f}"
            print_str = format_str.format(float(avg_loss),float(avg_category_loss),float(avg_confidence_loss),float(avg_class_loss), \
            ...
            # print(f"total loss {float(avg_loss):.3f}")
            # print(f"category_loss {float(avg_category_loss):.3f}")
            # print(f"confidence_loss", float(avg_confidence_loss))
            # print(f"class_loss", float(avg_class_loss))
            # print(f"instace_x_loss ", float(avg_instance_x_loss))
            # print(f"instace_y_loss ", float(avg_instance_y_loss))
            # print(f"heading_x_loss ", float(avg_heading_x_loss))
            # print(f"heading_y_loss ", float(avg_heading_y_loss))
            # print(f"height_loss ", float(avg_height_loss))
            #print(print_str)
            ic(print_str)

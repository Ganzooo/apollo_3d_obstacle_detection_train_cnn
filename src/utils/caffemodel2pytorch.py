...
def initialize(caffe_proto = 'https://raw.githubusercontent.com/BVLC/caffe/master/src/caffe/proto/caffe.proto', codegen_dir = tempfile.mkdtemp(), shadow_caffe = True):
	...
	if caffe_pb2 is None:
		...
		# subprocess.check_call(['protoc', '--proto_path', os.path.dirname(caffe_proto)])
...
if __name__ == '__main__':
	...
	parser.add_argument('--model', dest = 'model_caffemodel', help = 'Path to model.caffemodel', \
		default="/workspace/NETWORK/apollo_3d_obstacle_detection_train_cnn/checkpoints/apollo/deploy.caffemodel")
	parser.add_argument('-otput', dest = 'output_path', help = 'Path to converted model, supported file extensions are: h5, npy, npz, json, pt', \
		default="/workspace/NETWORK/apollo_3d_obstacle_detection_train_cnn/checkpoints/apollo/unet.pt")
	parser.add_argument('--proto', dest = 'caffe_proto', help = 'Path to caffe.proto (typically located at CAFFE_ROOT/src/caffe/proto/caffe.proto)', \
		default = "/workspace/NETWORK/apollo_3d_obstacle_detection_train_cnn/checkpoints/apollo/deploy.prototxt")

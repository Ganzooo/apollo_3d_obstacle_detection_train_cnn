...
class Trainer(object):
    ...
    def __init__(self, data_path, batch_size, max_epoch, pretrained_model,
        ...
        warmup = False
        if warmup:
           warmup_epochs = self.start_epo  +1
           ...
           self.scheduler.step()
            ...
            self.scheduler = StepLR(self.optimizer, step_size=step, gamma=0.5)
            for i in range(1, self.start_epo + 1):
                self.scheduler.step()
            #self.scheduler.step()
        new_lr = self.scheduler.get_last_lr()[0]
        ic("==> Training with start learning rate:", new_lr)

        ...
        category_loss \
            = torch.sum(
                category_weight * object_weight * 
                   torch.where(
                       category_diff <= sigma, 0.5 * (category_diff ** 2), sigma * 
                            torch.abs(category_diff) - 0.5 * (sigma ** 2)) * (1.0 + beta * input[:, 2, ...])) * 0.05
        ...
        confidence_loss \
            = torch.sum(
                confidence_weight * object_weight * torch.where(
                    ...
                    sigma * torch.abs(confidence_diff) - 0.5 * (sigma ** 2)) * (1.0 + beta * input[:, 2, ...])) * 0.005

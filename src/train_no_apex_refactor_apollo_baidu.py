...
class Trainer(object):
    def __init__(self, args,  logger, writer):
        ...
        if args.model_type == "unet":
            ...
            self.model = UNetWithResnet50Encoder(in_channels=self.in_channels, n_classes=5).to(self.device)
            ic(self.model)
            ic('ResNet Unet')
        else:
    ...
    def step_train(self, mode):
        ...
        if np.mod(self.epo, self.save_model_interval) == 0:
            ...
            _save_path_latest = osp.join(args.work_dir, '{}_latestmodel.pth'.format('glare'))
        ...
        self.writer.add_scalar("Loss/train/train_avg_loss", avg_loss, self.epo)
        self.writer.add_scalar("Loss/train/train_avg_confidence_loss", avg_confidence_loss, self.epo)
        self.writer.add_scalar("Loss/train/train_avg_category_loss", avg_category_loss, self.epo)
        self.writer.add_scalar("Loss/train/train_avg_class_loss", avg_class_loss, self.epo)
        self.writer.add_scalar("Loss/train/train_avg_instance_x_loss", avg_instance_x_loss, self.epo)
        self.writer.add_scalar("Loss/train/train_avg_instance_y_loss", avg_instance_y_loss, self.epo)
        self.writer.add_scalar("Loss/train/train_avg_heading_x_loss", avg_heading_x_loss, self.epo)
        self.writer.add_scalar("Loss/train/train_avg_heading_y_loss", avg_heading_y_loss, self.epo)
    ...
    def step_val(self, mode):
        ...
        if self.best_loss > loss_sum / len(self.val_dataloader):
            ...
            _save_path = osp.join(args.work_dir, '{}_bestmodel_{}.pth'.format('glare',str(float(self.best_loss))))
...
if __name__ == "__main__":
    ic.configureOutput(prefix='Lidar 3d obstacle detection |')
    ######### parser ###########
    args = options.Options().init(argparse.ArgumentParser(description='Lidar 3D obstacle detection')).parse_args()
    ic(args)
